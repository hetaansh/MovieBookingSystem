...
<form action="{{ route('admin.password.store') }}" method="post">
    @endif
    @if ($uname === 'operator')
    <form action="{{ route('operator.password.store') }}" method="post">
        @else
        <form action="{{ $password_reset_url }}" method="post">
            ...
            <div class="input-group mb-3">
                <input type="password" name="password_confirmation" class="form-control @error('password_confirmation') is-invalid @enderror" placeholder="{{ trans('adminlte::adminlte.retype_password') }}">
